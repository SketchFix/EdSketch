...
public class Explorer {
    ...
    public static void checkAssertion(boolean exp) throws BacktrackException {
		...
		if (m_multipleRuns) throw new BacktrackException();
		throw new RuntimeException();
    }

    public static boolean incrementCounter() {
    	if (m_counters.size() == 0) { //if there's not a counter at the end of an 
		                     //execution, then end the run
    		if (DEBUG) System.out.println("incrementCounter(): m_counter.size() == 0");	
    		return false;
    	}
		...
		while (m_counters.size() > 0) {
			...
			if ( last.getValue() == (last.getMax() - 1)) {
			    if (DEBUG) System.out.println("removed the last element reached at its max (" + last.getValue() + ")");
		...
		int i = 0;
		boolean atMax = true;
		if (DEBUG) System.out.println("There are " + m_counters.size() + " counters");
		/*for (int j = 0; j < m_counter.size(); j++) {
		    if (DEBUG) System.out.print("(" + get(j).getIndex() + ")" + get(j).getValue());
		}*/
		...
		while ( atMax && (i < m_counters.size())) {
		    if (get(i).getValue() != (get(i).getMax() - 1)) {
		    	if (DEBUG) System.out.println("Counter with max " + (get(i).getMax() - 1) + " has value " + get(i).getValue());
		    	atMax = false;
		    	break;
		    }
		...
		if (atMax) {
		    if (DEBUG) System.out.println("atMax, return");
		    return false;
		}
    ...
    /*private static int exists(int _index) {
	for (int i = 0; i < m_counter.size(); i++) {
	    if (get(i).getIndex() == _index) {
		return i;
	    }
	}
	return -1;
	...
	public static int choose(int _max) {
		...
		int value;
		/*if (vIndex < 0) {
		    m_counter.add(new Counter(_index, _max));
		    value = 0; //return the current value of the newly added counter, which is 0;
		}
		else {
		    value = get(vIndex).getValue();
		}*/
	
		if (m_counterPointer >= m_counters.size()) {   //add a new counter if reaches a new choose call
		    if (DEBUG) System.out.println("adding counter...");
